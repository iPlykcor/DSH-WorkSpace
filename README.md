...
- 📄 **Office 三件套内联预览**（新格式）：`.docx`（docx-preview 版式渲染）、`.xlsx`（SheetJS 解析 + 样式表格网格，含合并单元格）、`.pptx`（pptx 渲染器出幻灯片）。全部打进懒加载 `office` chunk（首开按需加载），离线、零系统依赖。详见 [CHANGELOG.md](./CHANGELOG.md)。
